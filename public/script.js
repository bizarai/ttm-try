...
// Initialize map with a placeholder token, will be replaced by the real token
let mapboxgl = {
  accessToken: ''
};

// Initialize map - will be created after fetching the token
let map;

// Add UI elements and get references
...
async function initializeMapWithSecureToken() {
  try {
    ...
    // After map is initialized, add user location support
    addUserLocationSupport();
    
  } catch (error) {
    console.error('Error initializing map:', error);
    const mapContainer = document.getElementById('map');
    mapContainer.innerHTML = '<div style="padding: 30px; text-align: center;">Failed to load map. Please refresh the page.</div>';
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  // First get the secure token and initialize the map
  initializeMapWithSecureToken();
  
  // Initialize autosuggest for search input
